package _2_object_and_class.lesson_5.homework;
...
public class Matrices {
    ...
    public static class Vector {
        ...
        public Vector add(Vector other) {
            int nLength = (int) this.length();
            if (nLength != other.length()) {
                throw new IllegalArgumentException("The lengths of the vectors are not equal");
            }

            int[] res = new int[nLength];

            for (int i = 0; i < nLength; i++) {
                int sum = items[i] + other.items[i];
                res[i] = sum;
            }
        ...
        public Vector subtract(Vector other) {
            int nLength = (int) this.length();
            if (nLength != other.length()) {
                throw new IllegalArgumentException("The length of the vectors are not equal");
            }

            int[] res = new int[nLength];

            for (int i = 0; i < nLength; i++) {
                int diff = items[i] - other.items[i];
                res[i] = diff;
            }
            return new Vector(res);
        }
            ...
            for (int i = 0; i < matrix.length; i++) {
                ...
                for (int iRow = 1; iRow < matrix.length; iRow++) {
                    ...
                    for (int iCol = 0; iCol < matrix.length; iCol++) {
                        if (iCol != i) {
                            int t = matrix[iRow][iCol];
                            smallerMatrix[iRow - 1][k] = t;
                            k++;
                        }
                    }
                }
